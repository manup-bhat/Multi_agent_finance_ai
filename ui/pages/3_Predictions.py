...
def get_predict(tkr, hrz):
    ...
    return {"ticker": tkr, "direction": "BULLISH", "confidence": 0.67, "regime": "BULL"}
...
if run_btn_val or prev != ticker or "pred_data" not in st.session_state:
    with st.spinner(f"Running prediction for {ticker}…"):
        ...
        df_p = safe_yf_download(ticker, period="3mo")
        last_price = float(df_p["Close"].iloc[-1]) if not df_p.empty else 1600.0
...
last_price = st.session_state.get("pred_price", 1600.0)
...
idx_future = pd.date_range(start=datetime.date.today(), periods=horizon+1, freq="B")
bull       = data.get("direction") == "BULLISH"
p50 = last_price * (1 + np.linspace(0, 0.04 if bull else -0.03, horizon+1))
p10 = p50 * (1 - np.linspace(0, 0.06, horizon+1))
p90 = p50 * (1 + np.linspace(0, 0.08, horizon+1))

fig = go.Figure()
fig.add_trace(go.Scatter(x=idx_hist, y=hist, name="Historical", line=dict(color="#8892b0",width=1.5)))
fig.add_trace(go.Scatter(x=idx_future, y=p90,  name="P90", line=dict(color="rgba(0,212,170,0.4)",width=1,dash="dot")))
fig.add_trace(go.Scatter(x=idx_future, y=p10,  name="P10", fill="tonexty",
                          fillcolor="rgba(0,212,170,0.07)", line=dict(color="rgba(0,212,170,0.4)",width=1,dash="dot")))
fig.add_trace(go.Scatter(x=idx_future, y=p50,  name="P50 (Median)", line=dict(color="#00d4aa",width=2.5)))
fig.add_vline(x=pd.Timestamp(datetime.date.today()).timestamp() * 1000, line_color="#f59e0b", line_dash="dash", annotation_text="Today")
...
probs_default = {"Very Bearish": 0.05, "Bearish": 0.12, "Neutral": 0.21, "Bullish": 0.41, "Very Bullish": 0.21}
probs  = data.get("class_probs", probs_default) or probs_default
